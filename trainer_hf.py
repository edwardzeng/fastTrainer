...
class Trainer:
    ...
    Args:
        ...
        model_init (`Callable[[], PreTrainedModel]`, *optional*):
            ...
            The function may have zero argument, or a single one containing the wandb trial object, to
    ...
    def __init__(
        ...
        if args.deepspeed and (self.optimizer is not None or self.lr_scheduler is not None):
            raise RuntimeError(
                "Passing `optimizers` is not allowed if Deepspeed is enabled."
                "You should subclass `Trainer` and override the `create_optimizer_and_scheduler` method."
            )
        ...
        if (args.fp16 or args.bf16) and not (args.deepspeed):
            # deepspeed Model Parallel manage their own half precision
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        """
        Launch an hyperparameter search using `wandb`. The optimized quantity is determined
    ...
    def _numpify_concat_truncate(self, input_host, output_all, num_samples=-1, is_nested=True):
        """nested_numpify data and concat it to output"""
        if input_host is None:
            return output_all

        input_host_numpify = nested_numpify(input_host)
        if output_all is None:
            output_all = input_host_numpify
        else:
            if is_nested:
                output_all = nested_concat(output_all, input_host_numpify, padding_index=-100)
            else:
                output_all = np.concatenate((output_all, input_host_numpify), axis=0)

        # Number of losses has been rounded to a multiple of batch_size and in a distributed training, the number of
        # samplers has been rounded to a multiple of batch_size, so we truncate.
        if num_samples != -1 and output_all is not None:
            if is_nested:
                output_all = nested_truncate(output_all, num_samples)
            else:
                output_all = output_all[:num_samples]
        return output_all
    
    def _pad_gather_concat(self, input_data, output, labels=None, batch_size=None, is_loss=False, is_logits=False):
        """pad data and nested_concat"""
        if input_data is None:
            return output
        
        if is_loss:
            input_data = self._nested_gather(input_data.repeat(batch_size))
            output = input_data if output is None else torch.cat((output, input_data), dim=0)
        else:
            inputs_data = self._pad_across_processes(inputs_data)
            inputs_data = self._nested_gather(inputs_data)
            if is_logits and self.preprocess_logits_for_metrics is not None:
                inputs_data = self.preprocess_logits_for_metrics(inputs_data, labels)
            output = inputs_data if output is None else nested_concat(output, inputs_data, padding_index=-100)
    ...
    def evaluation_loop(
        ...
        """
        Predict/eval loop, shared by Trainer.evaluate() and Trainer.predict(). Works both with or without labels.
        """
        args = self.args
        ...
        if args.deepspeed and not self.deepspeed:
            # eval doesn't have resume_from_checkpoint arg but we should be able to do eval from the checkpoint
            ds_engine, _, _ = deepspeed_init(self, num_training_steps=0, resume_from_checkpoint=None, inference=True)
            self.model = ds_engine.module
            self.model_wrapped = ds_engine
            self.deepspeed = ds_engine
        ...
        batch_size = self.args.eval_batch_size
        logger.info(f"Running {description}, num examples = {self.num_examples(dataloader)}, batch size = {batch_size}")
        ...
        # losses/preds/labels on GPU (accumulated for eval_accumulation_steps), all prefix is on CPU (final containers)
        losses_host, preds_host, labels_host, inputs_host = None, None, None, None
        all_losses, all_preds, all_labels, all_inputs = None, None, None, None
        ...
        for step, inputs in enumerate(dataloader):
            ...
            losses_host = self._pad_gather_concat(loss, losses_host, batch_size=batch_size, is_loss=True)
            labels_host = self._pad_gather_concat(labels, labels_host)
            inputs_host = self._pad_gather_concat(inputs_decode, inputs_host)
            preds_host = self._pad_gather_concat(logits, preds_host, labels, None, is_logits=True)

            self.control = self.callback_handler.on_prediction_step(args, self.state, self.control)

            # Gather all tensors and put them back on the CPU if we have done enough accumulation steps.
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                all_losses = self._numpify_concat_truncate(losses_host, all_losses, is_nested=False)
                all_preds = self._numpify_concat_truncate(preds_host, all_preds)
                all_inputs = self._numpify_concat_truncate(inputs_host, all_inputs)
                all_labels = self._numpify_concat_truncate(labels_host, all_labels)
        ...
        # Gather all remaining tensors and put them back on the CPU, truncate by num_samples
        num_samples = len(eval_dataset)
        all_losses = self._numpify_concat_truncate(losses_host, all_losses, num_samples=num_samples, is_nested=False)
        all_preds = self._numpify_concat_truncate(preds_host, all_preds, num_samples=num_samples)
        all_inputs = self._numpify_concat_truncate(inputs_host, all_inputs, num_samples=num_samples)
        all_labels = self._numpify_concat_truncate(labels_host, all_labels, num_samples=num_samples)
